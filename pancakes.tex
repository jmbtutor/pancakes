...
\subsection{Ending Your Turn}
\label{sec:endturn}

After you have completed your action (passing included), if there are any Servings on the Griddle, you may choose to take any number of them to add to your Tray for points if you win at the end. Any required flips must be done before Servings are taken.

A Serving is a set of four identically ranked cards face-up on the top of a single Stack.

\paragraph{} \label{par:takeserving}
Take the Serving and put it face-up on your Tray. Your Tray is the area in front of you where you can place your collected Servings.
...
\paragraph{} \label{par:servingpassbreak}
Taking a serving breaks the accumulation of passes. In particular, you cannot declare ``Pancakes!'' (\autoref{par:pancakeflip}) if you take a Serving.

\subsection{Winning}
\label{sec:winning}

The player who empties their hand first wins the game. If the game ends with everyone passing, the player with the fewest cards in their hand wins the game.
...
\begin{description}
  ...
  \item[Serving] (\autoref{sec:endturn})\\
...
\begin{enumerate}
  ...
  \item Take a Serving. (\autoref{sec:endturn})
